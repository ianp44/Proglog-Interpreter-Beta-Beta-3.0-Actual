...
This source code  has been registered on github (user name Ianp44) on 18/12/16.
...
var res;
if(labtab_top>=labtab_max-2){
sys_error("too many lable entries in add_labent");
}
res=labtab_top;
labtab[labtab_top]=labent;
labtab_top++;
return(res);
...
ruleind=labtab[ruleid].rule_index;
if(ruleind==-1){//new rulelist
ruleind=create_add_rule_list();
labtab[ruleid].rule_index=ruleind; //set up lable entry for
// that rule head id
}
...
function zset_stack_rule(lrule_obj){
var x;
var xtop;
var stack_obj;
...
stack_obj=compute_stack[compute_stack_top];
...
this.stack_list=new Array();
while(x<xtop){
this.stack_list[x]=new create_var_reference_obj(); //? #bug_alert, should be in ar array??
...
if(zchecks){
chk_is_stack_entry_object(cur_stack_obj,"create_stack_match_lvars_obj");
chk_is_stack_entry_object(new_compute_stack_obj,"create_stack_match_lvars_obj");
}
...
labtab_obj2=getlabtabent(f_id);
new_top_rule_index=labtab_obj2.rule_index;
...
function create_compute_stack_entry(f_id,l_link_up_index){
var x;
var xtop;
var lab_obj;
...
/*
if(zchecks){
chk_is_rule_obj(lrule_obj,"create_compute_stack_entry");
}
*/

this.ztype=stack_entry_object_type;
this.link_up_index=l_link_up_index; //stack posisition of calling compute stack entry
this.f_id=f_id;

this.cur_rule_obj=null;

//index into top rule list to work through
//##
...
//this.top_rule_index=top_rule_list_index;
this.top_rule_index=-1;

//this.top_rule_index=find_rule_list_index(f_id);
//lab_obj=getlabtabent(f_id);
...
function getlabtabent(f_id){
if((f_id<0)||(f_id>=labtab_top)){
sys_error("illegal value for f_id in getlabtabent.");
}
return(labtab[f_id]);
...
labtab_obj=getlabtabent(lf_id);
if(labtab_obj.typ==1){
this.is_variable=false;
}else if(labtab_obj.typ==2){
...
if(zchecks){
if((f1<0)||(f1>=labtab_top)){
sys_error("f1 out of range of labtab in ret_id_typ");

}
}
return(labtab[f1].typ);
}
